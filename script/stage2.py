...
@LastEditors: qc
@LastEditTime: 2020-06-24 18:42:53
@note:
    订足面 dzm 0.5 反向
    订足率 dzl 0.5 正向
    订单满足率 ddmz 0.8 反向
...
def get_guide(path):
    '''
    @description: 由路径得到三率一面、本期策略等指导数据
    @param：path
    @return: {三率一面,本期策略,品规名,本期订购总量,各组总户数}
    '''    
    dt = pd.read_csv(path, sep='\t')
    strategy = list(dt['strategy']) # 本期投放策略
    item = dt['item'][1] # 品规名
    dzm = list(dt['dzm'])
    dzl = list(dt['dzl'])
    ddmz = list(dt['ddmz'])
...
guide_data = get_guide('D:/jupyter_dir/yancao/data') # 读取品规数据
strategy = [] # 全局策略存储变量
